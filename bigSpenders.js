const businesses = [
  ...
  //==============================================================================================================
  // needed to use square bracket notation to display the state code
  const addressStateCode = "addressStateCode";

  // needed to use square bracket notation to display the zip code
  const addressZipCode = "addressZipCode";

  const outEl = document.querySelector("#output")
  outEl.innerHTML = "<h1><u>Active Businesses</u></h1>"
...
const manufacturingBusinesses = businesses.filter(business => {
  ...
  const manufacturingoutEl = document.querySelector("#manufacturingOutput")
  manufacturingoutEl.innerHTML = "<h1><u>Manufacturing</u></h1>"
  ...
  agentOutput.innerHTML = "<h1><u>Purchasing Agents</u></h1>";
...
agents.forEach(agent => {
  agentOutput.innerHTML += `<h2>${agent.fullName}</h2>
  <p>${agent.company}</p>
  <p>${agent.phoneNumber}</p>`
  agentOutput.innerHTML += "<hr/>";
});
...
const purchasingAgentSearch = document.querySelector("#purchasingAgentSearch")
purchasingAgentSearch.addEventListener("keypress", keyPressEvent => {
    if (keyPressEvent.charCode === 13) {
        /* WHEN  USER PRESSES ENTER, FIND MATCHING Agent */
        const foundAgent = businesses.find( business => business.purchasingAgent.nameFirst.includes(keyPressEvent.target.value) ||  business.purchasingAgent.nameLast.includes(keyPressEvent.target.value)
            
        );
        outputEl.innerHTML ="<h2>Results:</h2>"
        outputEl.innerHTML += `
            <h2>
...
//==============================================================================================================
// Display businesses and sales with forEach
//==============================================================================================================
...
//REDUCE()
//==============================================================================================================
// 1: Use the reduce method on the following array to determine how much total rain fell last month.
const monthlyRainfall = [23, 13, 27, 20, 20, 31, 33, 26, 19, 12, 14, 12, 10]

const totalRainfall = monthlyRainfall.reduce((total, currentRainfall,) => total + currentRainfall, 0)
